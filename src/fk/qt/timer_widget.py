...
class TimerWidget(QtCore.QObject):
    ...
    def show(self) -> None:
        if self._widget is None:
            raise Exception("Cannot show a Pixmap")
        self._widget.show()
...
def render_for_widget(ref_palette: QtGui.QPalette, widget: QtWidgets.QWidget, font: QtGui.QFont, thickness: float) -> TimerWidget:
    ...
    widget.installEventFilter(result)
    widget.show()
    return result


def render_for_logo(widget: QtWidgets.QWidget, thickness: float) -> TimerWidget:
    result = TimerWidget(
        widget,
        QtGui.QColor("#000000"),
        QtGui.QColor("#000000"),
...
def render_for_pixmap() -> TimerWidget:
    result = TimerWidget(
        None,
        None,
        None,
