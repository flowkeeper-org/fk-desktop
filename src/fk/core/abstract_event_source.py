...
class AbstractEventSource(AbstractEventEmitter, ABC, Generic[TRoot]):
    ...
    def execute_prepared_strategy(self, strategy: AbstractStrategy[TRoot], auto: bool = False, persist: bool = False) -> None:
        ...
        # UC: All executed strategies are wrapped in BeforeMessageProcessed / AfterMessageProcessed events
        ...
        if res is not None and res[0] == 'auto-seal':
            ...
            # UC: Certain strategies may request the "auto-seal" BEFORE they execute
            self.auto_seal()
            res = strategy.execute(self._emit, self.get_data())
            if res is not None and res[0] == 'auto-seal':
        ...
        if persist:
            ...
            # UC: Strategy sequence is incremented only after it is persisted
    ...
    def execute(self,
        ...
        s = strategy_class(
            self._last_seq + 1,
            when,
            self._settings.get_username(),  # UC: Strategy owner is taken from the source settings
            params,
            self._settings,
            carry)
