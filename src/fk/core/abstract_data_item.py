...
def generate_unique_name(prefix: str, names: Iterable) -> str:
    # UC: An incremental index is appended to the new backlog / WI name, if there's an existing duplicate
    check = prefix
    n = 1
...
class AbstractDataItem(ABC, Generic[TParent]):
    ...
    def get_owner(self) -> 'User':
        # UC: All data objects are owned by a User, or delegated to their parent
    ...
    def item_updated(self, date: datetime.datetime = None):
        # UC: Update timestamps propagate to parents. The latest timestamp is kept, they can't decrease.
        if date is None:
